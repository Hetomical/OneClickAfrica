    ...
    public function home()
    {
        ...
        if (Sentinel::check()):
            ...
            if($primarySection->status == 1):
                ...
                $primarySectionPosts    = Cache::remember('primarySectionPostsAuth', $seconds = 1, function () {
            ...
            $sliderPosts            = Cache::remember('sliderPostsAuth', $seconds = 1, function () {
                return  Post::with(['category', 'image', 'user'])
                    ->where('visibility', 1)
                    ->where('slider', 1)
                    ->where('status', 1)
            ...
            $categorySections       = Cache::remember('categorySectionsAuth', $seconds = 1, function () {
            ...
            $video_posts     = Cache::remember('video_postsAuth', $seconds = 1, function () {
                return Post::with('category', 'image', 'user')
            ...
            $latest_posts       = Cache::remember('latest_postsAuth', $seconds = 1, function () {
                return Post::with('category', 'image', 'user')
                    ->where('visibility', 1)
                    ->where('status', 1)
                    ->orderBy('id', 'desc')
            ...
            $totalPostCount     = Cache::remember('totalPostCountAuth', $seconds = 1, function () {
            ...
            if(!isNull($primarySection)):
                if($primarySection->status == 1):
                    ...
                    $primarySectionPosts    = Cache::remember('primarySectionPosts', $seconds = 1, function () {
                        return Post::with(['category', 'image', 'user'])
                            ->where('visibility', 1)
                            ->where('status', 1)
                            ->where('slider', '!=', 1)
            ...
            $sliderPosts            = Cache::remember('sliderPosts', $seconds = 1, function () {
                return  Post::with(['category', 'image', 'user'])
                    ->where('visibility', 1)
                    ->where('slider', 1)
                    ->where('status', 1)
            ...
            $categorySections       = Cache::remember('categorySections', $seconds = 1, function () {
                return ThemeSection::with('ad')
                    ->with(['category'])
                    ->where('is_primary', '<>', 1)->orderBy('order', 'ASC')
                    ->where(function ($query) {
            ...
            $video_posts     = Cache::remember('video_posts', $seconds = 1, function () {
                return Post::with('category', 'image', 'user')
                    ->where('post_type', 'video')
                    ->where('visibility', 1)
                    ->where('status', 1)
            ...
            $latest_posts       = Cache::remember('latest_posts', $seconds = 1, function () {
                return Post::with('category', 'image', 'user')
                    ->where('visibility', 1)
                    ->where('status', 1)
                    ->when(Sentinel::check() == false, function ($query) {
            ...
            $totalPostCount     = Cache::remember('totalPostCount', $seconds = 1, function () {
                return Post::where('visibility', 1)
                    ->where('status', 1)
                    ->when(Sentinel::check() == false, function ($query) {
                        $query->where('auth_required', 0);
